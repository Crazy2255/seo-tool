...
<div class="py-6">
        ...
        <div class="md:flex md:items-center md:justify-between">
            <div class="flex-1 min-w-0">
                ...
                <p class="mt-1 text-sm text-gray-500">
                    Create a new marketing campaign to generate leads and track performance
                </p>
            </div>
            <div class="mt-4 flex md:mt-0 md:ml-4">
                <a href="{{ route('campaigns.index') }}" class="inline-flex items-center px-4 py-2 border border-gray-300 rounded-md shadow-sm text-sm font-medium text-gray-700 bg-white hover:bg-gray-50">
                    Back to Campaigns
                </a>
            </div>
        </div>
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mt-6">
        <form action="{{ route('campaigns.store') }}" method="POST" enctype="multipart/form-data" class="bg-white shadow-sm rounded-lg">
            @csrf
                ...
                <div class="grid grid-cols-1 gap-6">
                    ...
                    <div>
                        ...
                        <select id="campaign_type" name="campaign_type"
                            ...
                            <option value="general" {{ old('campaign_type') == 'general' ? 'selected' : '' }}>General</option>
                            <option value="email" {{ old('campaign_type') == 'email' ? 'selected' : '' }}>Email</option>
                            <option value="social" {{ old('campaign_type') == 'social' ? 'selected' : '' }}>Social Media</option>
                            <option value="affiliate" {{ old('campaign_type') == 'affiliate' ? 'selected' : '' }}>Affiliate</option>
                            <option value="custom" {{ old('campaign_type') == 'custom' ? 'selected' : '' }}>Custom</option>
                        </select>
                        @error('campaign_type')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
        ...
        <div class="bg-white shadow-sm rounded-lg">
            <div class="px-4 py-5 sm:p-6">
                ...
                <div class="mt-2 max-w-xl text-sm text-gray-500">
                    <p>After creating your campaign, you'll be able to send bulk emails or generate a shareable link.</p>
                </div>
            </div>
        </div>
    </div>
